...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    // Only enable it on macOS where we use vibrancy
    transparent: process.platform === "darwin",
    backgroundColor: process.platform === "darwin" ? "#00000000" : "#0B0B0F", // Dark background for Windows to prevent white flash
    icon: getIconPath(),
    webPreferences: {
  ...
  // Add platform-specific vibrancy/acrylic effects
  if (process.platform === "darwin") {
