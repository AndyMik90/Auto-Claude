...
export function initSentryMain(): void {
  ...
  if (!hasDsn) {
    console.debug('[Sentry] No SENTRY_DSN configured - error reporting disabled');
    console.debug('[Sentry] To enable: set SENTRY_DSN environment variable');
  }
  ...
  ipcMain.on(IPC_CHANNELS.SENTRY_STATE_CHANGED, (_event, enabled: boolean) => {
    ...
    console.debug(`[Sentry] Error reporting ${enabled ? 'enabled' : 'disabled'} (via IPC)`);
  });
  ...
  if (hasDsn) {
    console.debug(`[Sentry] Main process initialized (enabled: ${sentryEnabledState}, traces: ${cachedTracesSampleRate}, profiles: ${cachedProfilesSampleRate})`);
  }
}
...
export function setSentryEnabled(enabled: boolean): void {
  sentryEnabledState = enabled;
  console.debug(`[Sentry] Error reporting ${enabled ? 'enabled' : 'disabled'} (programmatic)`);
}
