...
async function validateClaudeCliAsync(cliPath: string): Promise<[boolean, string | null]> {
  try {
    ...
    if (isWindowsPlatform && !isSecurePath(cliPath)) {
    ...
    if (isWindowsPlatform && /\.(cmd|bat)$/i.test(cliPath)) {
...
async function scanClaudeInstallations(activePath: string | null): Promise<ClaudeInstallationInfo[]> {
  ...
  if (!isWindowsPlatform && existsSync(detectionPaths.nvmVersionsDir)) {
    try {
  ...
  const additionalPaths = isWindowsPlatform
