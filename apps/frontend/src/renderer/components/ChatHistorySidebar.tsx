...
export function ChatHistorySidebar({
  ...
  const formatDate = (date: Date) => {
    ...
    if (diffDays === 0) {
      return t('insights:history.today');
    } else if (diffDays === 1) {
      return t('insights:history.yesterday');
    } else if (diffDays < 7) {
      return t('insights:history.daysAgo', { count: diffDays });
    } else {
  ...
  return (
    <div className="flex h-full w-64 flex-col border-r border-border bg-muted/30">
      ...
      <div className="flex items-center justify-between border-b border-border px-3 py-3">
        <h3 className="text-sm font-medium text-foreground">{t('insights:history.title')}</h3>
        <Tooltip>
      ...
      <AlertDialog open={!!deleteSessionId} onOpenChange={() => setDeleteSessionId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('insights:history.deleteTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('insights:history.deleteDescription')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="flex items-center gap-1.5 pr-7">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            {t('insights:history.messagesCount', { count: session.messageCount })}
          </p>
        </div>
      </div>
      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end" sideOffset={5} className="w-36 z-[100]">
          <DropdownMenuItem onSelect={onStartEdit}>
            ...
            {t('common:accessibility.renameAriaLabel')}
          </DropdownMenuItem>
          <DropdownMenuItem
            onSelect={onDelete}
