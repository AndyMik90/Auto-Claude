      ...
      <DialogContent className="sm:max-w-[500px]">
            ...
            <div className="rounded-lg bg-success/10 border border-success/30 p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-success">
                  {t('auth.tokenConfigured')}
                </p>
                <p className="text-xs text-success/80 mt-1">
                  {t('auth.tokenConfiguredHint')}
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t('auth.existingAccountsHint', { count: claudeProfiles.length })}
                      </p>
                    </div>
                  </div>
                </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-foreground">
                            {profile.name}
                            {profile.isDefault && (
                              <span className="ml-2 text-xs text-muted-foreground">{t('auth.default')}</span>
                            )}
                          </p>
                          {profile.email && (
              ...
              <div className="space-y-3">
                <div className="rounded-lg bg-info/10 border border-info/30 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-foreground font-medium mb-1">
                        {claudeProfiles.length > 0 ? t('auth.orAuthNew') : t('auth.authWithBrowser')}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {claudeProfiles.length > 0
                          ? t('auth.authNewDescription')
                ...
                >
                  {isAuthenticating ? (
                    ...
                    <>
                      ...
                      {claudeProfiles.length > 0 ? t('auth.authNewButton') : t('auth.authWithBrowser')}
                    </>
                  )}
                </Button>
              ...
              {showManualEntry && (
                <div className="space-y-3 pl-4 border-l-2 border-border">
                  ...
                  <div className="text-xs text-muted-foreground space-y-1">
                    <p className="font-medium text-foreground">{t('auth.steps')}</p>
            ...
            {hasExistingToken && (
              <div className="rounded-lg bg-muted/50 p-3">
                <p className="text-sm text-muted-foreground">
                  {t('auth.existingTokenInfo')} {showManualEntry ? t('auth.replaceTokenHint') : t('auth.replaceAuthHint')}
                </p>
              </div>
            )}
          </div>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={handleClose} disabled={isSaving || isAuthenticating}>
            {success ? t('auth.close') : t('auth.cancel')}
          </Button>
