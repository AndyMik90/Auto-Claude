...
export function EnvConfigModal({
  ...
  const handleSave = async () => {
    if (!token.trim()) {
      setError(t('envConfig.pleaseEnterToken'));
      return;
    }
        ...
        {!isChecking && !success && (
          <div className="py-4 space-y-4">
              ...
              <div className="space-y-3">
                <div className="rounded-lg bg-info/10 border border-info/30 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-foreground font-medium mb-1">
                        {claudeProfiles.length > 0 ? t('envConfig.orAuthenticateNew') : t('envConfig.authenticateWithBrowser')}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {claudeProfiles.length > 0
                          ? t('envConfig.addNewAccount')
                          : t('envConfig.clickToLogin')
                        }
                      </p>
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              ...
              >
                <span>{t('envConfig.enterManually')}</span>
                {showManualEntry ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        placeholder={t('envConfig.enterTokenPlaceholder')}
            ...
            {hasExistingToken && (
              <div className="rounded-lg bg-muted/50 p-3">
                <p className="text-sm text-muted-foreground">
                  {t('envConfig.tokenAlreadyConfigured')} {showManualEntry ? t('envConfig.enterNewToken') : t('envConfig.authenticateAgain')}
                </p>
              </div>
            )}
          </div>
