          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('tasksSelected', { selected: selectedTaskIds.length, total: doneTasks.length })}
              </span>
            ...
            {doneTasks.length === 0 ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('noCompletedTasks')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {t('noCompletedTasksDescription')}
                  </p>
                </div>
              </div>
            ) : (
      ...
      {(sourceMode === 'git-history' || sourceMode === 'branch-diff') && (
        <>
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('commitsFound', { count: previewCommits.length })}
              </span>
              {isLoadingCommits && (
            ...
            {isLoadingCommits ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <p className="mt-4 text-sm text-muted-foreground">{t('loadingCommits')}</p>
                </div>
              </div>
              ...
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('noCommitsFound')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {sourceMode === 'git-history'
                      ? t('noCommitsFoundGitHistory')
                      : t('noCommitsFoundBranchDiff')}
                  </p>
                </div>
              </div>
            ) : (
