...
export function ChangelogList({
  ...
  const getSummaryLabel = () => {
    ...
    switch (sourceMode) {
      case 'tasks':
        return count === 1 ? t('list.task') : t('list.tasks');
      case 'git-history':
      case 'branch-diff':
        return count === 1 ? t('list.commit') : t('list.commits');
      default:
        return '';
    }
  };
                ...
                <div className="text-center py-12">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {sourceMode === 'git-history'
                      ? t('list.noCommitsHintHistory')
                      : t('list.noCommitsHintBranch')}
                  </p>
                </div>
              </div>
            ) : (
