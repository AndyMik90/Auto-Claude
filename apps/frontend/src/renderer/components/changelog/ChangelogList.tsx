          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('list.taskMode.selectedCount', { selected: selectedTaskIds.length, total: doneTasks.length })}
              </span>
            ...
            {doneTasks.length === 0 ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('list.taskMode.noTasks')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {t('list.taskMode.noTasksDescription')}
                  </p>
                </div>
              </div>
            ) : (
            ...
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {previewCommits.length === 1
                  ? t('list.gitMode.commitsFoundSingular', { count: previewCommits.length })
                  : t('list.gitMode.commitsFound', { count: previewCommits.length })}
              </span>
              {isLoadingCommits && (
            ...
            {isLoadingCommits ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <p className="mt-4 text-sm text-muted-foreground">{t('list.gitMode.loadingCommits')}</p>
                </div>
              </div>
              ...
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('list.gitMode.noCommits')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {sourceMode === 'git-history'
                      ? t('list.gitMode.noCommitsGitHistory')
                      : t('list.gitMode.noCommitsBranchDiff')}
                  </p>
                </div>
              </div>
            ) : (
