...
export function ChangelogList({
  ...
  const getSummaryLabel = () => {
    ...
    switch (sourceMode) {
      case 'tasks':
        return count !== 1 ? t('commits.tasks') : t('commits.task');
      case 'git-history':
      case 'branch-diff':
        return count !== 1 ? t('commits.commits') : t('commits.commit');
      default:
        return 'item';
    }
  };
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('selection.selected', { count: selectedTaskIds.length, total: doneTasks.length })}
              </span>
            ...
            {isLoadingCommits ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <p className="mt-4 text-sm text-muted-foreground">{t('commits.loading')}</p>
                </div>
              </div>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {sourceMode === 'git-history'
                      ? t('hints.configureHistory')
                      : t('hints.selectBranches')}
                  </p>
                </div>
              </div>
            ) : (
