    ...
    <div className="w-80 shrink-0 border-r border-border overflow-y-auto">
      <div className="p-6 space-y-6">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-sm">
                    {t('filters.sourceMode.tasks.label')}
                  </span>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {t('filters.sourceMode.tasks.description')}
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-sm">
                    {t('filters.sourceMode.gitHistory.label')}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {t('filters.sourceMode.gitHistory.description')}
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-sm">
                    {t('filters.sourceMode.branchDiff.label')}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {t('filters.sourceMode.branchDiff.description')}
                </p>
              </div>
            </label>
          </RadioGroup>
        ...
        {sourceMode === 'git-history' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('filters.gitHistory.title')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* History Type */}
              <div className="space-y-2">
                <Label className="text-xs">{t('filters.gitHistory.historyType')}</Label>
                <Select
                  ...
                  <SelectContent>
                    <SelectItem value="since-version">
                      <div className="flex items-center gap-2">
                        ...
                        {t('filters.gitHistory.types.sinceVersion')}
                      </div>
                    </SelectItem>
                    <SelectItem value="recent">
                      <div className="flex items-center gap-2">
                        <History className="h-3 w-3" />
                        {t('filters.gitHistory.types.recent')}
                      </div>
                    </SelectItem>
                    <SelectItem value="since-date">
                      <div className="flex items-center gap-2">
                        <Calendar className="h-3 w-3" />
                        {t('filters.gitHistory.types.sinceDate')}
                      </div>
                    </SelectItem>
                    <SelectItem value="tag-range">
                      <div className="flex items-center gap-2">
                        <Tag className="h-3 w-3" />
                        {t('filters.gitHistory.types.tagRange')}
                      </div>
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              {gitHistoryType === 'recent' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('filters.gitHistory.numberOfCommits')}</Label>
                  <Input
                    type="number"
                    min={1}
                    max={500}
              ...
              {gitHistoryType === 'since-date' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('filters.gitHistory.sinceDate')}</Label>
                  <Input
                    type="date"
              ...
              {gitHistoryType === 'tag-range' && (
                <>
                  <div className="space-y-2">
                    <Label className="text-xs">{t('filters.gitHistory.fromTag')}</Label>
                    <Select value={gitHistoryFromTag} onValueChange={onGitHistoryFromTagChange}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('filters.gitHistory.selectVersion')} />
                      </SelectTrigger>
                      <SelectContent>
                        {tags.map((tag) => (
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs">{t('filters.gitHistory.toTag')}</Label>
                    <Select value={gitHistoryToTag || 'HEAD'} onValueChange={(v) => onGitHistoryToTagChange(v === 'HEAD' ? '' : v)}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('filters.gitHistory.head')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="HEAD">{t('filters.gitHistory.head')}</SelectItem>
                        {tags.map((tag) => (
                          <SelectItem key={tag.name} value={tag.name}>
                            {tag.name}
                          </SelectItem>
              ...
              {gitHistoryType === 'since-version' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('filters.gitHistory.lastVersion')}</Label>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t('filters.gitHistory.versionHelp')}
                  </p>
                </div>
              )}
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="merge-commits" className="text-xs cursor-pointer">
                  {t('filters.gitHistory.includeMergeCommits')}
                </Label>
              </div>
              ...
              >
                {isLoadingCommits ? (
                  <>
                    ...
                    {t('filters.gitHistory.loading')}
                  </>
                ) : (
                  <>
                    <RefreshCw className="mr-2 h-4 w-4" />
                    {t('filters.gitHistory.loadCommits')}
                  </>
                )}
              </Button>
            </CardContent>
        ...
        {sourceMode === 'branch-diff' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('filters.branchComparison.title')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label className="text-xs">{t('filters.branchComparison.baseBranch')}</Label>
                <Select value={baseBranch} onValueChange={onBaseBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('filters.branchComparison.selectBaseBranch')} />
                  </SelectTrigger>
                  <SelectContent>
                      ...
                      <SelectItem key={branch.name} value={branch.name}>
                        <div className="flex items-center gap-2">
                          ...
                          {branch.name === defaultBranch && (
                            <Badge variant="outline" className="text-xs">{t('filters.branchComparison.default')}</Badge>
                          )}
                        </div>
                      </SelectItem>
                    ))}
                ...
                <p className="text-xs text-muted-foreground">
                  {t('filters.branchComparison.baseBranchHelp')}
                </p>
              </div>

              <div className="space-y-2">
                <Label className="text-xs">{t('filters.branchComparison.compareBranch')}</Label>
                <Select value={compareBranch} onValueChange={onCompareBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('filters.branchComparison.selectCompareBranch')} />
                  </SelectTrigger>
                  <SelectContent>
                    {localBranches.map((branch) => (
                      <SelectItem key={branch.name} value={branch.name}>
                        <div className="flex items-center gap-2">
                          {branch.name}
                          {branch.isCurrent && (
                            <Badge variant="secondary" className="text-xs">{t('filters.branchComparison.current')}</Badge>
                          )}
                        </div>
                      </SelectItem>
                    ))}
                ...
                <p className="text-xs text-muted-foreground">
                  {t('filters.branchComparison.compareBranchHelp')}
                </p>
              </div>
              ...
              {baseBranch && compareBranch && baseBranch === compareBranch && (
                <div className="flex items-center gap-2 text-destructive text-xs">
                  ...
                  {t('filters.branchComparison.branchesMustBeDifferent')}
                </div>
              )}
