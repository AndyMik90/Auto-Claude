      ...
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('competitorViewer.title')}
          </DialogTitle>
          <DialogDescription>
            {t('competitorViewer.description', { count: analysis.competitors.length })}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6 pb-4">
              ...
              >
                ...
                <div>
                  <h4 className="text-sm font-medium mb-2 flex items-center gap-2">
                    ...
                    {t('competitorViewer.painPointsTitle')} ({competitor.painPoints.length})
                  </h4>
                  <div className="space-y-2">
                    {competitor.painPoints.length === 0 ? (
                      <p className="text-sm text-muted-foreground italic">
                        {t('competitorViewer.noPainPoints')}
                      </p>
                    ) : (
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1">
                              ...
                              {painPoint.source && (
                                <div className="mt-2">
                                  <span className="text-xs text-muted-foreground">
                                    {t('competitorViewer.source')} <span className="italic">{painPoint.source}</span>
                                  </span>
                                </div>
                              )}
                              {painPoint.frequency && (
                                <div className="mt-1">
                                  <span className="text-xs text-muted-foreground">
                                    {t('competitorViewer.frequency')} {painPoint.frequency}
                                  </span>
                                </div>
                              )}
                              {painPoint.opportunity && (
                                <div className="mt-1">
                                  <span className="text-xs text-muted-foreground">
                                    {t('competitorViewer.opportunity')}{' '}
            ...
            {analysis.insightsSummary && (
              <div className="rounded-lg bg-primary/5 border border-primary/20 p-4 space-y-3">
                <h4 className="text-sm font-semibold">{t('competitorViewer.insightsSummary')}</h4>

                {analysis.insightsSummary.topPainPoints.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-muted-foreground mb-1">{t('competitorViewer.topPainPoints')}</p>
                ...
                {analysis.insightsSummary.differentiatorOpportunities.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-muted-foreground mb-1">{t('competitorViewer.differentiatorOpportunities')}</p>
                    <ul className="text-sm space-y-1">
                ...
                {analysis.insightsSummary.marketTrends.length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-muted-foreground mb-1">{t('competitorViewer.marketTrends')}</p>
                    <ul className="text-sm space-y-1">
