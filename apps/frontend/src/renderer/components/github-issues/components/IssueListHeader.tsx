    ...
    <div className="shrink-0 p-4 border-b border-border">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-xs">
            {t('issuesList.openCount', { count: openIssuesCount })}
          </Badge>
          <Button
            variant="ghost"
            size="icon"
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  {t('issuesList.analyzeGroupButton')}
                </Button>
              </TooltipTrigger>
              <TooltipContent side="bottom" className="max-w-xs">
                <p>{t('issuesList.analyzeGroupTooltip')}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
          ...
          <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/50 border border-border">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="max-w-xs">
                  <p>{t('issuesList.autoFixTooltip')}</p>
                  {autoFixRunning && autoFixProcessing !== undefined && autoFixProcessing > 0 && (
                    <p className="mt-1 text-primary">{t('issuesList.processing', { count: autoFixProcessing })}</p>
                  )}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
      ...
      <div className="flex items-center gap-3">
        ...
        <Select value={filterState} onValueChange={onFilterChange}>
          ...
          <SelectContent>
            <SelectItem value="open">{t('issuesList.filters.open')}</SelectItem>
            <SelectItem value="closed">{t('issuesList.filters.closed')}</SelectItem>
            <SelectItem value="all">{t('issuesList.filters.all')}</SelectItem>
          </SelectContent>
        </Select>
      </div>
    </div>
