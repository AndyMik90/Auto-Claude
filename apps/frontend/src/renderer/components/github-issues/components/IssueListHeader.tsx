    ...
    <div className="shrink-0 p-4 border-b border-border">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-xs">
            {openIssuesCount} {t('github:issues.open')}
          </Badge>
          <Button
            variant="ghost"
            size="icon"
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  {t('github:issues.batchReview.button')}
                </Button>
              </TooltipTrigger>
              <TooltipContent side="bottom" className="max-w-xs">
                <p>{t('github:issues.batchReview.tooltip')}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
          ...
          <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/50 border border-border">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor="auto-fix-toggle" className="text-sm cursor-pointer whitespace-nowrap">
                      {t('github:issues.autoFix.autoFixNew')}
                    </Label>
                    <Switch
                      id="auto-fix-toggle"
                ...
                <TooltipContent side="bottom" className="max-w-xs">
                  <p>{t('github:issues.autoFix.autoFixNewTooltip')}</p>
                  {autoFixRunning && autoFixProcessing !== undefined && autoFixProcessing > 0 && (
                    <p className="mt-1 text-primary">{t(`github:issues.autoFix.processingCount${autoFixProcessing > 1 ? '_plural' : ''}`, { count: autoFixProcessing })}</p>
                  )}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <Input
            placeholder={t('github:issues.searchPlaceholder')}
            value={searchQuery}
