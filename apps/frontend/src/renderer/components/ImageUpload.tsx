...
export function ImageUpload({
  ...
  const processFiles = useCallback(
    async (files: FileList | File[]) => {
      ...
      if (remainingSlots <= 0) {
        setError(t('imageUpload.maxReached'));
        return;
      }
      ...
      for (const file of filesToProcess) {
        ...
        if (!isValidImageType(file)) {
          errors.push(t('imageUpload.invalidType', { filename: file.name, types: ALLOWED_IMAGE_TYPES_DISPLAY }));
          continue;
        }

        // Warn about large files
        if (file.size > MAX_IMAGE_SIZE) {
          errors.push(t('imageUpload.tooLarge', { filename: file.name }));
