  ...
  const processFiles = useCallback(
    async (files: FileList | File[]) => {
      ...
      if (fileArray.length > remainingSlots) {
        setError(t('errors.filesSkipped', { count: remainingSlots }));
      }
      ...
      for (const file of filesToProcess) {
        ...
        if (!isValidImageType(file)) {
          errors.push(t('errors.invalidFileType', { fileName: file.name, allowed: ALLOWED_IMAGE_TYPES_DISPLAY }));
          continue;
        }

        // Warn about large files
        if (file.size > MAX_IMAGE_SIZE) {
          errors.push(t('errors.fileToolarge', { fileName: file.name }));
        ...
        try {
          ...
          errors.push(t('errors.fileProcessFailed', { fileName: file.name }));
        }
      }
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            {canAddMore
              ...
              : t('imageUpload.maxMessage', { max: MAX_IMAGES_PER_TASK })}
          </p>
        </div>
      </div>
