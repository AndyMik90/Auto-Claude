    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Card className="p-3 text-center">
          ...
          <div className="text-xs text-muted-foreground">{t('ideation:details.severity')}</div>
        </Card>
        <Card className="p-3 text-center">
          <div className={`text-lg font-semibold ${IDEATION_EFFORT_COLORS[idea.estimatedEffort]}`}>
            {idea.estimatedEffort}
          </div>
          <div className="text-xs text-muted-foreground">{t('ideation:details.effort')}</div>
        </Card>
      </div>
      ...
      {idea.breakingChange && (
        <div className="rounded-lg bg-destructive/10 border border-destructive/30 p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-destructive">{t('ideation:details.breakingChange')}</span>
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            {t('ideation:details.breakingChangeWarning')}
          </p>
        </div>
      )}
      ...
      <div>
        <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
          ...
          {t('ideation:details.currentState')}
        </h3>
      ...
      <div>
        <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
          ...
          {t('ideation:details.proposedChange')}
        </h3>
      ...
      {idea.codeExample && (
        <div>
          <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
            ...
            {t('ideation:details.codeExample')}
          </h3>
      ...
      {idea.metrics && (
        <div>
          <h3 className="text-sm font-medium mb-2">{t('ideation:details.metrics')}</h3>
          <div className="grid grid-cols-2 gap-2">
            {idea.metrics.lineCount && (
              <Card className="p-2 text-center">
                <div className="text-sm font-semibold">{idea.metrics.lineCount}</div>
                <div className="text-xs text-muted-foreground">{t('ideation:details.lines')}</div>
              </Card>
            )}
            {idea.metrics.complexity && (
              <Card className="p-2 text-center">
                <div className="text-sm font-semibold">{idea.metrics.complexity}</div>
                <div className="text-xs text-muted-foreground">{t('ideation:details.complexity')}</div>
              </Card>
            )}
            {idea.metrics.duplicateLines && (
              <Card className="p-2 text-center">
                <div className="text-sm font-semibold">{idea.metrics.duplicateLines}</div>
                <div className="text-xs text-muted-foreground">{t('ideation:details.duplicateLines')}</div>
              </Card>
            )}
            {idea.metrics.testCoverage !== undefined && (
              <Card className="p-2 text-center">
                <div className="text-sm font-semibold">{idea.metrics.testCoverage}%</div>
                <div className="text-xs text-muted-foreground">{t('ideation:details.testCoverage')}</div>
              </Card>
            )}
          </div>
        </div>
      ...
      {idea.affectedFiles && idea.affectedFiles.length > 0 && (
        <div>
          <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
            <FileCode className="h-4 w-4" />
            {t('ideation:details.affectedFiles')}
          </h3>
      ...
      {idea.bestPractice && (
        <div>
          <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
            ...
            {t('ideation:details.bestPractice')}
          </h3>
      ...
      {idea.prerequisites && idea.prerequisites.length > 0 && (
        <div>
          <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
            ...
            {t('ideation:details.prerequisites')}
          </h3>
          <ul className="space-y-1">
