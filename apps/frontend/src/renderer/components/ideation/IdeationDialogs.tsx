  ...
  return (
    <>
      ...
      <Dialog open={showConfigDialog} onOpenChange={onCloseConfigDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('dialogs.config.title')}</DialogTitle>
            <DialogDescription>
              {t('dialogs.config.description')}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4 max-h-96 overflow-y-auto">
            <div className="space-y-3">
              <h4 className="text-sm font-medium">{t('dialogs.config.ideationTypes')}</h4>
              {ALL_IDEATION_TYPES.map((type) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-sm">{t(`types.${type.replace(/_/g, '-')}`)}</div>
            ...
            <div className="space-y-3">
              <h4 className="text-sm font-medium">{t('dialogs.config.contextSources')}</h4>
              <div className="flex items-center justify-between">
                <span className="text-sm">{t('dialogs.config.includeRoadmap')}</span>
                <Switch
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm">{t('dialogs.config.includeKanban')}</span>
                <Switch
      ...
      <Dialog open={showAddMoreDialog} onOpenChange={onCloseAddMoreDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('dialogs.addMore.title')}</DialogTitle>
            <DialogDescription>
              {t('dialogs.addMore.description')}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-3 max-h-96 overflow-y-auto">
            {availableTypesToAdd.length === 0 ? (
              <div className="text-center py-6 text-muted-foreground">
                ...
                <p>{t('dialogs.addMore.allGenerated')}</p>
                <p className="text-sm mt-1">{t('dialogs.addMore.useRegenerate')}</p>
              </div>
            ) : (
          ...
          <DialogFooter className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              {typesToAdd.length > 0 && `${typesToAdd.length} ${t('dialogs.addMore.selected')}`}
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={onCloseAddMoreDialog}>
                {t('dialogs.addMore.cancel')}
              </Button>
              <Button
              ...
              >
                ...
                {t('dialogs.addMore.generate')} {typesToAdd.length > 0 ? `${typesToAdd.length} ${t('dialogs.addMore.types')}` : t('dialogs.addMore.ideas')}
              </Button>
            </div>
          </DialogFooter>
        </DialogContent>
