...
export function TerminalGrid({ projectPath, onNewTaskClick, isActive = false }: TerminalGridProps) {
  ...
  if (terminals.length === 0) {
    const shortcut = navigator.platform.includes('Mac') ? '⌘+T' : 'Ctrl+T';
    return (
      <div className="flex h-full flex-col items-center justify-center gap-6 p-8">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">{t('terminal:grid.agentTerminals')}</h2>
            <p className="mt-1 text-sm text-muted-foreground max-w-md">
              {t('terminal:grid.agentTerminalsDescription', { shortcut })}
            </p>
          </div>
        </div>
        ...
        <div className="flex h-10 items-center justify-between border-b border-border bg-card/30 px-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-muted-foreground">
              {t('terminal:grid.terminalsCount', { count: terminals.length })}
            </span>
          </div>
          <div className="flex items-center gap-2">
