...
export function AgentProfiles() {
  ...
  const getThinkingLabel = (thinkingValue: string): string => {
    ...
    return level ? t(`thinkingLevels.${level.value}.label`) : thinkingValue;
  };
      ...
      >
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-foreground">{t(`presets.${profile.id}.name`)}</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              {t(`presets.${profile.id}.description`)}
            </p>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <span className="inline-flex items-center rounded-md bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground">
                {getThinkingLabel(profile.thinkingLevel)} {t('thinking')}
              </span>
            </div>
          </div>
        </div>
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 border-b border-border bg-background px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">{t('label')}</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {t('presets.auto.description')}
            </p>
          </div>
        </div>
      </div>
