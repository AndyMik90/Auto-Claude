  ...
  const handleInitializeGit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('gitSetup.initFailed'));
        setStep('info');
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t('gitSetup.initFailed'));
      setStep('info');
    } finally {
      ...
      <div className="py-4 space-y-4">
        ...
        <details className="text-sm">
          ...
          <div className="mt-3 rounded-lg bg-muted/50 p-3 font-mono text-xs space-y-1">
            ...
            {needsGitInit && <p>{t('gitSetup.commands.init')}</p>}
            <p>{t('gitSetup.commands.add')}</p>
            <p>{t('gitSetup.commands.commit')}</p>
          </div>
        </details>
