...
// Phase labels are now translated via t('logs.phases.planning') etc.
      ...
      <CollapsibleTrigger asChild>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-sm">{t(`logs.phases.${phase}`)}</span>
            {hasEntries && (
              <span className="text-xs text-muted-foreground">
                ({phaseLog?.entries.length} {t('logs.entries')})
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
