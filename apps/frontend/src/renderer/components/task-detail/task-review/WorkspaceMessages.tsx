  ...
  return (
    <div className="rounded-xl border border-border bg-secondary/30 p-4">
      <h3 className="font-medium text-sm text-foreground mb-2 flex items-center gap-2">
        ...
        {t('taskReview:workspaceMessages.noWorkspaceFound')}
      </h3>
      <p className="text-sm text-muted-foreground mb-3">
        {t('taskReview:workspaceMessages.noWorkspaceHint')}
      </p>
        ...
        >
          {isMarkingDone ? (
            <>
              ...
              {t('taskReview:workspaceMessages.updating')}
            </>
          ) : (
            <>
              <Check className="h-4 w-4 mr-2" />
              {t('taskReview:workspaceMessages.markAsDone')}
            </>
          )}
        </Button>
      )}
...
export function StagedInProjectMessage({ task, projectPath, hasWorktree = false, onClose }: StagedInProjectMessageProps) {
  ...
  const stagedDate = task.stagedAt ? ` ${new Date(task.stagedAt).toLocaleDateString()}` : '';

  return (
    <div className="rounded-xl border border-success/30 bg-success/10 p-4">
      <h3 className="font-medium text-sm text-foreground mb-2 flex items-center gap-2">
        <GitMerge className="h-4 w-4 text-success" />
        {t('taskReview:workspaceMessages.changesStagedInProject')}
      </h3>
      <p className="text-sm text-muted-foreground mb-3">
        {t('taskReview:workspaceMessages.changesStagedHint', { date: stagedDate })}
      </p>
      <div className="bg-background/50 rounded-lg p-3 mb-3">
        <p className="text-xs text-muted-foreground mb-2">{t('taskReview:workspaceMessages.nextSteps')}</p>
        <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
          <li>{t('taskReview:workspaceMessages.step1', { statusCommand: '<code>git status</code>', diffCommand: '<code>git diff --staged</code>' })}</li>
          <li>{t('taskReview:workspaceMessages.step2', { commitCommand: '<code>git commit -m "your message"</code>' })}</li>
          <li>{t('taskReview:workspaceMessages.step3')}</li>
        </ol>
      </div>
      ...
      {hasWorktree && (
        <div className="flex flex-col gap-2">
          <div className="flex gap-2">
            ...
            >
              {isDeleting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  {t('taskReview:workspaceMessages.cleaningUp')}
                </>
              ) : (
                <>
                  <Check className="h-4 w-4 mr-2" />
                  {t('taskReview:workspaceMessages.deleteWorktreeMarkDone')}
                </>
              )}
            </Button>
          </div>
          ...
          <p className="text-xs text-muted-foreground">
            {t('taskReview:workspaceMessages.cleanupHint')}
          </p>
        </div>
      )}
    </div>
