      ...
      {suggestedCommitMessage && (
        <div className="bg-background/50 rounded-lg p-3 mb-3">
          ...
          <p className="text-[10px] text-muted-foreground mt-1.5">
            {t('staged.editNote')} <code className="bg-background px-1 rounded">git commit -m "..."</code>
          </p>
        </div>
      )}

      <div className="bg-background/50 rounded-lg p-3">
        <p className="text-xs text-muted-foreground mb-2">{t('staged.nextStepsTitle')}</p>
        <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
          <li>{t('staged.step1')}</li>
          <li>{t('staged.step2')} <code className="bg-background px-1 rounded">git status</code> {t('staged.step2And')} <code className="bg-background px-1 rounded">git diff --staged</code></li>
          <li>{t('staged.step3')} <code className="bg-background px-1 rounded">git commit -m "your message"</code></li>
        </ol>
      </div>
    </div>
  );
