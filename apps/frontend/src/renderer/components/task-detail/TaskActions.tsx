    ...
    <>
      <div className="p-4">
        ...
        {task.status === 'done' && (
          <div className="completion-state text-sm">
            ...
            <span className="font-medium">{t('actions.taskCompleted')}</span>
          </div>
        )}
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={onShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="text-sm text-muted-foreground space-y-3">
                <p>
                  {t('actions.deleteDialog.confirmMessage', { title: task.title })}
                </p>
                <p className="text-destructive">
                  {t('actions.deleteDialog.warningMessage')}
                </p>
                {deleteError && (
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeleting}>{t('common.cancel', { ns: 'common' })}</AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault();
                onDelete();
