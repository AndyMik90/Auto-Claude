      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge variant="outline">{t(`status.${roadmap.status}`)}</Badge>
            {competitorAnalysis && (
              <Tooltip>
                ...
                <TooltipContent className="max-w-md">
                  <div className="space-y-2">
                    <div className="font-semibold">{t('header.competitorAnalysisTooltip')}</div>
                    <div className="text-sm text-muted-foreground">
                      {t('header.analyzedCompetitors', {
        ...
        <div className="flex items-center gap-2">
          <Tooltip>
            ...
            <TooltipContent>{t('header.addFeatureTooltip')}</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
            ...
            <TooltipContent>{t('header.regenerateTooltip')}</TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      {roadmap.targetAudience && (
        <div className="mt-4 flex items-center gap-4 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">{t('summary.target')}</span>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="text-muted-foreground cursor-help underline decoration-dotted">
                  {t('summary.morePersonas', { count: roadmap.targetAudience.secondary.length })}
                </div>
              </TooltipTrigger>
              <TooltipContent className="max-w-md">
                <div className="space-y-1">
                  <div className="font-semibold mb-2">{t('summary.secondaryPersonas')}</div>
      ...
      <div className="mt-4 flex items-center gap-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">
            ...
            <span className="text-muted-foreground"> {t('summary.features')}</span>
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm">
            <span className="font-semibold">{roadmap.phases.length}</span>
            <span className="text-muted-foreground"> {t('summary.phases')}</span>
          </span>
        </div>
