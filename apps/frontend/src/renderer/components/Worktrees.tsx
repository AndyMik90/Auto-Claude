...
export function Worktrees({ projectId }: WorktreesProps) {
  const { t } = useTranslation(['terminal', 'common', 'dialogs']);
  ...
  if (!selectedProject) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-muted-foreground">{t('terminal:worktree.selectProject')}</p>
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground flex items-center gap-2">
            ...
            {t('terminal:worktree.title')}
          </h2>
          <p className="text-sm text-muted-foreground mt-1">
            {t('terminal:worktree.subtitle')}
          </p>
        </div>
      ...
      {!isLoading && worktrees.length === 0 && terminalWorktrees.length === 0 && (
        <div className="flex h-full flex-col items-center justify-center text-center">
          ...
          <h3 className="text-lg font-semibold text-foreground">{t('terminal:worktree.noWorktrees')}</h3>
          <p className="text-sm text-muted-foreground mt-2 max-w-md">
            {t('terminal:worktree.noWorktreesDescription')}
          </p>
        </div>
      )}
                      ...
                      <CardContent className="pt-0">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm mb-4">
                          <div className="flex items-center gap-1.5 text-muted-foreground">
                            ...
                            <span>{worktree.filesChanged} {t('terminal:worktree.filesChanged')}</span>
                          </div>
                          <div className="flex items-center gap-1.5 text-muted-foreground">
                            <ChevronRight className="h-3.5 w-3.5" />
                            <span>{worktree.commitCount} {t('terminal:worktree.commitsAhead')}</span>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          >
                            ...
                            {t('terminal:worktree.mergeTo', { branch: worktree.baseBranch })}
                          </Button>
                          {task && (
                            <Button
                              variant="info"
                    ...
                    <CardContent className="pt-0">
                      ...
                      {wt.createdAt && (
                        <div className="text-xs text-muted-foreground mb-4">
                          {t('terminal:worktree.created', { date: new Date(wt.createdAt).toLocaleDateString() })}
                        </div>
                      )}
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('terminal:worktree.mergeWorktree')}
            </DialogTitle>
            <DialogDescription>
              {t('terminal:worktree.mergeDescription')}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="py-4">
              <div className="rounded-lg bg-muted p-4 text-sm space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('terminal:worktree.sourceBranch')}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('terminal:worktree.targetBranch')}</span>
                  <span className="font-mono">{selectedWorktree.baseBranch}</span>
                </div>
                <div className="border-t border-border pt-3 mt-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">{t('terminal:worktree.changes')}</span>
                    <span>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className={`font-medium ${mergeResult.success ? 'text-success' : 'text-destructive'}`}>
                      {mergeResult.success ? t('terminal:worktree.mergeSuccessful') : t('terminal:worktree.mergeFailed')}
                    </p>
                    <p className="text-muted-foreground mt-1">{mergeResult.message}</p>
                    {mergeResult.conflictFiles && mergeResult.conflictFiles.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-medium">{t('terminal:worktree.conflictingFiles')}</p>
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('terminal:worktree.deleteWorktreeTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('terminal:worktree.deleteWorktreeDescription')}
              {worktreeToDelete && (
                <span className="block mt-2 font-mono text-sm">
                  {worktreeToDelete.branch}
                </span>
              )}
              {t('terminal:worktree.cannotUndo')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={!!terminalWorktreeToDelete} onOpenChange={(open) => !open && setTerminalWorktreeToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('terminal:worktree.deleteTerminalWorktreeTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('terminal:worktree.deleteTerminalWorktreeDescription')}
