...
export function GitHubOAuthFlow({ onSuccess, onCancel }: GitHubOAuthFlowProps) {
  ...
  const handleAuthTimeout = useCallback(() => {
    ...
    setIsTimeout(true);
    setError(t('githubOAuth.authTimeout'));
    setStatus('error');
    authTimeoutRef.current = null;
  }, [t]);
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-3">
                  <h3 className="text-lg font-medium text-foreground">
                    {t('githubOAuth.cliRequired')}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    {t('githubOAuth.cliRequiredDescription')}
                  </p>
                  <div className="flex gap-3">
                    <Button onClick={handleOpenGhInstall} className="gap-2">
                      <ExternalLink className="h-4 w-4" />
                      {t('githubOAuth.installCli')}
                    </Button>
                    <Button variant="outline" onClick={handleRetry}>
                      {t('githubOAuth.installedIt')}
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 text-sm text-muted-foreground">
                  <p className="font-medium text-foreground mb-2">{t('githubOAuth.installInstructions')}</p>
          ...
          <Card className="border border-info/30 bg-info/10">
            <CardContent className="p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-3">
                  <h3 className="text-lg font-medium text-foreground">
                    {t('githubOAuth.connectToGitHub')}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    {t('githubOAuth.connectDescription')}
                  </p>
                  {cliVersion && (
                    <p className="text-xs text-muted-foreground">
                      {t('githubOAuth.usingCli', { version: cliVersion })}
                    </p>
                  )}
                </div>
              </div>
          ...
          <Card className="border border-info/30 bg-info/10">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-medium text-foreground">
                    {t('githubOAuth.authenticating')}
                  </h3>
                  <p className="text-sm text-muted-foreground mt-1">
                    {browserOpened
                      ? t('githubOAuth.completeInBrowser')
                      : t('githubOAuth.waitingForAuth')}
                  </p>
                </div>
              </div>
            </CardContent>
              ...
              <CardContent className="p-6">
                <div className="text-center space-y-4">
                  ...
                  <div className="text-sm text-muted-foreground space-y-2">
                    <p>
                      {browserOpened
                        ? t('githubOAuth.enterCodeInBrowser')
                        : t('githubOAuth.copyCodeThenOpen')}
                    </p>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-medium text-success">
                  {t('githubOAuth.successfullyConnected')}
                </h3>
                <p className="text-sm text-success/80 mt-1">
                  {username ? t('githubOAuth.connectedAs', { username }) : t('githubOAuth.accountConnected')}
                </p>
              </div>
            </div>
          </CardContent>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className={`text-lg font-medium ${isTimeout ? 'text-warning' : 'text-destructive'}`}>
                    {isTimeout ? t('githubOAuth.authTimedOut') : t('githubOAuth.authFailed')}
                  </h3>
              ...
              <CardContent className="p-5">
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="text-base font-medium text-foreground">
                        {t('githubOAuth.completeManually')}
                      </h3>
                      <p className="text-sm text-muted-foreground mt-1">
                        {t('githubOAuth.browserFailedDescription')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    >
                      <ExternalLink className="h-4 w-4" />
                      {t('githubOAuth.openUrlInBrowser')}
                    </Button>
                  </div>
