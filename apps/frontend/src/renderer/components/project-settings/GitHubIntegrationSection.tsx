...
export function GitHubIntegrationSection({
  ...
  const badge = envConfig.githubEnabled ? (
    <StatusBadge status="success" label={t('githubIntegration.statusEnabled')} />
  ) : null;
    ...
    >
        ...
        <div className="rounded-lg border border-info/30 bg-info/5 p-3 mb-4">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-foreground">{t('githubIntegration.projectSpecificTitle')}</p>
              <p className="text-xs text-muted-foreground mt-1">
                <Trans i18nKey="settings:githubIntegration.projectSpecificDescription" values={{ projectName }}>
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          <Label className="font-normal text-foreground">{t('githubIntegration.enableLabel')}</Label>
        ...
        <>
          ...
          {envConfig.githubAuthMethod === 'oauth' && envConfig.githubToken && !showOAuthFlow ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('githubIntegration.authTitle')}</Label>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <div className="flex items-center gap-2 p-3 rounded-lg border border-success/30 bg-success/5">
                ...
                <span className="text-sm text-foreground">{t('githubIntegration.authenticatedViaOAuth')}</span>
              </div>
            </div>
          ) : showOAuthFlow ? (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('githubIntegration.patTitle')}</Label>
                <Button
                  variant="outline"
                  size="sm"
              ...
              <p className="text-xs text-muted-foreground">
                <Trans i18nKey="settings:githubIntegration.patInstructions">
              ...
              <PasswordInput
                ...
                placeholder={t('githubIntegration.patPlaceholder')}
              />
            </div>
          )}

          <div className="space-y-2">
            <Label className="text-sm font-medium text-foreground">{t('githubIntegration.repoTitle')}</Label>
            <p className="text-xs text-muted-foreground">
              <Trans i18nKey="settings:githubIntegration.repoFormat">
          ...
          {envConfig.githubToken && envConfig.githubRepo && (
            <ConnectionStatus
              ...
              title={t('githubIntegration.connectionTitle')}
              successMessage={t('githubIntegration.connectionSuccess', { repoFullName: gitHubConnectionStatus?.repoFullName })}
              errorMessage={gitHubConnectionStatus?.error || t('githubIntegration.connectionError')}
          ...
          {gitHubConnectionStatus?.connected && (
            <div className="rounded-lg border border-info/30 bg-info/5 p-3">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-foreground">{t('githubIntegration.issuesAvailableTitle')}</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {t('githubIntegration.issuesAvailableDescription')}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <div className="flex items-center gap-2">
                ...
                <Label className="font-normal text-foreground">{t('githubIntegration.autoSyncLabel')}</Label>
              </div>
