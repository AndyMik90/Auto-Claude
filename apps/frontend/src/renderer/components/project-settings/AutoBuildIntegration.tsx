        ...
        <div className="rounded-lg border border-border bg-muted/50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-foreground">{t('autoBuildIntegration.notInitialized')}</p>
              <p className="text-xs text-muted-foreground mt-1">
                {t('autoBuildIntegration.initializeDescription')}
              </p>
              <Button
                size="sm"
                className="mt-3"
              ...
              >
                {isUpdating ? (
                  <>
                    ...
                    {t('autoBuildIntegration.initializing')}
                  </>
                ) : (
                  <>
                    <Download className="mr-2 h-4 w-4" />
                    {t('autoBuildIntegration.initializeButton')}
                  </>
                )}
              </Button>
            </div>
        ...
        <div className="rounded-lg border border-border bg-muted/50 p-4 space-y-3">
          ...
          {isCheckingVersion ? (
            ...
            <div className="text-xs text-muted-foreground">
              {versionInfo.isInitialized ? t('autoBuildIntegration.initialized') : t('autoBuildIntegration.notInitialized')}
            </div>
          )}
        </div>
      )}
