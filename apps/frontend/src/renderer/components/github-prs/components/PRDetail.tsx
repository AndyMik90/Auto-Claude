...
export function PRDetail({
  ...
  const handlePostCleanReview = async () => {
    ...
    try {
      // Format the clean review comment
...
*This automated review found no issues. Generated by Auto Claude.*`;
      await onPostComment(cleanReviewMessage);
      // Only mark as posted on success, clear any error
      setCleanReviewPosted(true);
      setCleanReviewError(null);
    } catch (err) {
      // Set error message for display to user
      const errorMessage = err instanceof Error ? err.message : 'Failed to post clean review comment';
      setCleanReviewError(errorMessage);
