...
import { Code, Terminal, RefreshCw, Loader2, Check, FolderOpen, AlertTriangle, Type } from 'lucide-react';
...
type DevToolsSubSection = 'tools' | 'terminal-fonts';
...
export function DevToolsSettings({ settings, onSettingsChange }: DevToolsSettingsProps) {
  ...
  // Track which sub-section is active
  const [activeSubSection, setActiveSubSection] = useState<DevToolsSubSection>('tools');
  ...
  return (
    <SettingsSection
      ...
      description={t('devtools.description', 'Configure your preferred IDE, terminal, and terminal font settings')}
    >
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 p-1 rounded-lg bg-muted">
          ...
          >
            ...
            <span>{t('devtools.tabTools', 'Tools')}</span>
          </button>
          <button
            type="button"
            onClick={() => setActiveSubSection('terminal-fonts')}
            className={cn(
              'flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-colors',
              activeSubSection === 'terminal-fonts'
                ? 'bg-background text-foreground shadow-sm'
                : 'text-muted-foreground hover:text-foreground hover:bg-accent/50'
            )}
          >
            <Type className="h-4 w-4" />
            <span>{t('devtools.tabTerminalFonts', 'Terminal Fonts')}</span>
          </button>
        </div>
