  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground">{t('theme.selectColorPalette')}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-1">
          {COLOR_THEMES.map((theme) => {
            ...
            // Use translated theme name and description if available
            const themeName = t(`theme.themes.${theme.id}.name`, { defaultValue: theme.name });
            const themeDescription = t(`theme.themes.${theme.id}.description`, { defaultValue: theme.description });
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="font-medium text-sm text-foreground">{themeName}</p>
                  <p className="text-xs text-muted-foreground line-clamp-2">{themeDescription}</p>
                </div>
              </button>
            );
          })}
