      ...
      {envConfig.githubEnabled && (
        <>
              ...
              <div className="rounded-lg border border-success/30 bg-success/10 p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-success">{t('projectSections.github.connectedViaCLI')}</p>
                      {oauthUsername && (
          ...
          {authMode === 'oauth' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('projectSections.github.githubAuthentication')}</Label>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleSwitchToManual}
          ...
          {authMode === 'manual' && (
            <>
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('projectSections.github.createTokenWithScope')}{' '}
                  <a
          ...
          <p className="text-xs text-muted-foreground">
            {isChecking ? t('projectSections.github.checking') :
              connectionStatus?.connected
                ? t('projectSections.github.connectedTo', { repo: connectionStatus.repoFullName })
...
function BranchSelector({
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative pl-6">
        ...
        >
          {isLoading ? (
            <span className="flex items-center gap-2 text-muted-foreground">
              <Loader2 className="h-4 w-4 animate-spin" />
              {t('projectSections.github.loadingBranches')}
            </span>
          ) : selectedBranch ? (
            <span className="flex items-center gap-2">
        ...
        {isOpen && !isLoading && (
          <div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-64 overflow-hidden">
            ...
            >
              <span className="text-sm text-muted-foreground italic">{t('projectSections.github.autoDetect')}</span>
            </button>
            ...
            <div className="max-h-40 overflow-y-auto border-t border-border">
              {filteredBranches.length === 0 ? (
                <div className="px-3 py-4 text-sm text-muted-foreground text-center">
                  {filter ? t('projectSections.github.noMatchingBranches') : t('projectSections.github.noBranchesFound')}
                </div>
              ) : (
      ...
      {selectedBranch && (
        <p className="text-xs text-muted-foreground pl-6">
          {t('projectSections.github.tasksWillBranchFrom')} <code className="px-1 bg-muted rounded">{selectedBranch}</code>
        </p>
      )}
    </div>
  );
