    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          <Label className="font-normal text-foreground">{t('githubIntegration.enableGithubIssues')}</Label>
          <p className="text-xs text-muted-foreground">
            {t('githubIntegration.syncIssuesDescription')}
          </p>
        </div>
        <Switch
      ...
      {envConfig.githubEnabled && (
        <>
              ...
              <div className="rounded-lg border border-success/30 bg-success/10 p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-success">{t('githubIntegration.connectedViaGitHubCli')}</p>
                      {oauthUsername && (
          ...
          {authMode === 'oauth' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium text-foreground">{t('githubIntegration.githubAuthentication')}</Label>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleSwitchToManual}
          ...
          {authMode === 'manual' && (
            <>
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('githubIntegration.createTokenDescription')}{' '}<code className="px-1 bg-muted rounded">repo</code>
                </p>
                <PasswordInput
...
function RepositoryDropdown({
  ...
  return (
    <div className="space-y-2">
        ...
        >
          {isLoading ? (
            <span className="flex items-center gap-2 text-muted-foreground">
              ...
              {t('githubIntegration.loadingRepositories')}
            </span>
          ) : selectedRepo ? (
            ...
            <div className="max-h-48 overflow-y-auto">
              {filteredRepos.length === 0 ? (
                <div className="px-3 py-4 text-sm text-muted-foreground text-center">
                  {filter ? t('githubIntegration.noMatchingRepositories') : t('githubIntegration.noRepositoriesFound')}
                </div>
              ) : (
      ...
      {selectedRepo && (
        <p className="text-xs text-muted-foreground">
          {t('githubIntegration.selected')} <code className="px-1 bg-muted rounded">{selectedRepo}</code>
        </p>
      )}
    </div>
  );
...
function RepositoryInput({ value, onChange }: RepositoryInputProps) {
  ...
  return (
    <div className="space-y-2">
      <Label className="text-sm font-medium text-foreground">{t('githubIntegration.repository')}</Label>
      <p className="text-xs text-muted-foreground">
        {t('githubIntegration.repositoryFormatDescription')}
      </p>
      <Input
        placeholder={t('githubIntegration.repositoryPlaceholder')}
        value={value}
          ...
          <p className="text-xs text-muted-foreground">
            {isChecking ? t('githubIntegration.checking') :
              connectionStatus?.connected
                ? t('githubIntegration.connectedTo', { repoName: connectionStatus.repoFullName })
    ...
    <div className="flex items-center justify-between">
      <div className="space-y-0.5">
        <div className="flex items-center gap-2">
          ...
          <Label className="font-normal text-foreground">{t('githubIntegration.autoSyncOnLoad')}</Label>
        </div>
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground pl-6">
            {t('githubIntegration.baseBranchDescription')}
          </p>
        </div>
        <Button
          variant="ghost"
      ...
      <div className="relative pl-6">
        ...
        >
          {isLoading ? (
            ...
            <span className="text-muted-foreground">{t('githubIntegration.autoDetectBranch')}</span>
          )}
          <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${isOpen ? 'rotate-180' : ''}`} />
        </button>
        ...
        {isOpen && !isLoading && (
          <div className="absolute z-50 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-64 overflow-hidden">
            ...
            >
              <span className="text-sm text-muted-foreground italic">{t('githubIntegration.autoDetectBranch')}</span>
            </button>
      ...
      {selectedBranch && (
        <p className="text-xs text-muted-foreground pl-6">
          {t('githubIntegration.allTasksWillBranchFrom')} <code className="px-1 bg-muted rounded">{selectedBranch}</code>
        </p>
      )}
    </div>
  );
