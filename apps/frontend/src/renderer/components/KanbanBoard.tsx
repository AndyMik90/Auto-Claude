 ...
 * stopped tasks are displayed in the 'backlog' column.
...
function getVisualColumn(status: TaskStatus): typeof TASK_STATUS_COLUMNS[number] {
  ...
  if (status === 'stopped') return 'backlog';
  return status;
}
...
export function KanbanBoard({ tasks, onTaskClick, onNewTaskClick, onRefresh, isRefreshing }: KanbanBoardProps) {
  ...
  const tasksByStatus = useMemo(() => {
    ...
    // Note: stopped tasks are shown in the 'backlog' column
    ...
    filteredTasks.forEach((task) => {
      // Map pr_created tasks to the done column, stopped tasks to backlog
