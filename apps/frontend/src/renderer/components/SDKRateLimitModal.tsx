...
function getSourceName(source: SDKRateLimitInfo['source'], t: any): string {
  switch (source) {
    case 'changelog': return t('sources.changelog', { ns: 'rateLimit' });
    case 'task': return t('sources.task', { ns: 'rateLimit' });
    case 'roadmap': return t('sources.roadmap', { ns: 'rateLimit' });
    case 'ideation': return t('sources.ideation', { ns: 'rateLimit' });
    case 'title-generator': return t('sources.titleGenerator', { ns: 'rateLimit' });
    default: return t('sources.default', { ns: 'rateLimit' });
  }
}
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (initResult.success) {
          ...
          alert(
            t('alert.title', { name: profileName }) + '\n\n' +
            t('alert.steps') + '\n' +
            t('alert.step1') + '\n' +
            t('alert.step2') + '\n' +
            t('alert.step3') + '\n\n' +
          ...
          alert(t('alert.failed', { error: initResult.error || '' }));
        }
      }
    } catch (err) {
      console.error('Failed to add profile:', err);
      alert(t('alert.addFailed'));
    } finally {
      setIsAddingProfile(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-[520px]">
        <DialogHeader>
          ...
          <DialogDescription className="flex items-center gap-2">
            ...
            {sourceName} {t('interrupted')}
            {currentProfile && (
              <span className="text-muted-foreground"> ({t('profile')} {currentProfile.name})</span>
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 space-y-4">
            ...
            {swapInfo?.wasAutoSwapped ? (
              <>
                <p className="font-medium mb-1">
                  {swapInfo.swapReason === 'proactive' ? t('swap.proactive') : t('swap.reactive')}
                </p>
                <p>
                  {swapInfo.swapReason === 'proactive'
                    ? t('swap.proactiveDescription', { from: swapInfo.swappedFrom, to: swapInfo.swappedTo })
                    : t('swap.reactiveDescription', { from: swapInfo.swappedFrom, to: swapInfo.swappedTo })
                  }
                </p>
                <p className="mt-2 text-[10px]">
                  {t('swap.continuedWithoutInterruption')}
                </p>
              </>
            ) : (
              <>
                <p className="font-medium mb-1">{t('swap.rateLimitReached')}</p>
                <p>
                  {t('swap.operationStopped', { profile: currentProfile?.name || 'your account' })}
                  {hasMultipleProfiles
                    ? ' ' + t('swap.switchBelow')
                    : ' ' + t('swap.addAccount')}
                </p>
            ...
            <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/50 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-foreground">
                  {t('reset.title', { time: sdkRateLimitInfo.resetTime })}
                </p>
          ...
          <div className="rounded-lg border border-accent/50 bg-accent/10 p-4">
            <h4 className="text-sm font-medium text-foreground mb-2 flex items-center gap-2">
              ...
              {hasMultipleProfiles ? t('switching.title') : t('switching.useAnother')}
            </h4>
            ...
            {hasMultipleProfiles ? (
              <>
                <p className="text-sm text-muted-foreground mb-3">
                  {suggestedProfile ? (
                    <>{t('switching.recommended', { profile: suggestedProfile.name })}</>
                  ) : (
                    t('switching.switchTo')
                  )}
                </p>
                    ...
                    <DropdownMenuTrigger asChild>
                      <Button variant="outline" className="flex-1 justify-between">
                        <span className="truncate flex items-center gap-2">
                          {selectedProfile?.name || t('switching.selectAccount')}
            ...
            <div className={hasMultipleProfiles ? "mt-4 pt-3 border-t border-border/50" : ""}>
              <p className="text-xs text-muted-foreground mb-2">
                {hasMultipleProfiles ? t('switching.addAnotherPrompt') : t('switching.connectPrompt')}
              </p>
              <div className="flex items-center gap-2">
                <Input
                  placeholder={t('switching.accountNamePlaceholder')}
          ...
          <div className="text-xs text-muted-foreground bg-muted/30 rounded-lg p-3">
            ...
            <p>
              {t('info.description', { source: sourceName.toLowerCase(), profile: currentProfile?.name || 'Default' })}
              {hasMultipleProfiles
                ? ' ' + t('info.switchOrAdd')
                : ' ' + t('info.addOrWait')}
            </p>
