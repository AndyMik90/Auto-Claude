      ...
      <div className="p-6 space-y-6">
          ...
          {searchResults.length > 0 && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                {t('memories.search.resultsFound', { count: searchResults.length })}
              </p>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              {t('memories.browser.memoriesCount', { filtered: filteredMemories.length, total: recentMemories.length })}
            </span>
          </div>
            ...
            {(Object.keys(memoryFilterCategories) as FilterCategory[]).map((category) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{t(`memories.filters.${category}`)}</span>
                  {count > 0 && (
                    <Badge
                      variant="secondary"
                      className={cn(
          ...
          {!memoriesLoading && filteredMemories.length === 0 && recentMemories.length > 0 && (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <Brain className="h-10 w-10 text-muted-foreground mb-3" />
              <p className="text-sm text-muted-foreground">
                {t('memories.browser.noFilterMatch')}
              </p>
              <Button
                variant="link"
                size="sm"
