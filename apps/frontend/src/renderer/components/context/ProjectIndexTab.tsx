      ...
      <div className="p-6 space-y-6">
        ...
        {indexError && (
          <div className="flex items-center gap-3 p-4 rounded-lg bg-destructive/10 text-destructive">
            ...
            <div>
              <p className="font-medium">{t('projectIndex.loadError')}</p>
        ...
        {!indexLoading && !projectIndex && !indexError && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-foreground">{t('projectIndex.noIndex.title')}</h3>
            <p className="text-sm text-muted-foreground mt-2 max-w-sm">
              {t('projectIndex.noIndex.description')}
            </p>
            <Button onClick={onRefresh} className="mt-4">
              <RefreshCw className="h-4 w-4 mr-2" />
              {t('projectIndex.noIndex.analyzeButton')}
            </Button>
          </div>
        )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {Object.keys(projectIndex.services).length > 0 && (
                    <Badge variant="secondary">
                      {Object.keys(projectIndex.services).length} {t('projectIndex.service')}
                    </Badge>
                  )}
                </div>
