...
export function MemoryCard({ memory }: MemoryCardProps) {
  ...
  const typeLabel = t(`types.${memory.type}`, { defaultValue: memory.type.replace(/_/g, ' ') });

  const sessionLabel = memory.session_number
    ? t('card.session', { number: memory.session_number })
    : parsed?.session_number
      ? t('card.session', { number: parsed.session_number })
      : null;
        ...
        {expanded && parsed && (
          <div className="mt-4 space-y-4 pt-4 border-t border-border/50">
            ...
            {parsed.what_worked && parsed.what_worked.length > 0 && (
              <div>
                <SectionHeader icon={CheckCircle2} title={t('card.whatWorked')} count={parsed.what_worked.length} />
            ...
            {parsed.what_failed && parsed.what_failed.length > 0 && (
              <div>
                <SectionHeader icon={XCircle} title={t('card.whatFailed')} count={parsed.what_failed.length} />
                <ul className="space-y-0.5">
            ...
            {parsed.discoveries?.patterns_discovered && parsed.discoveries.patterns_discovered.length > 0 && (
              <div>
                <SectionHeader icon={Sparkles} title={t('card.patterns')} count={parsed.discoveries.patterns_discovered.length} />
            ...
            {parsed.discoveries?.gotchas_discovered && parsed.discoveries.gotchas_discovered.length > 0 && (
              <div>
                <SectionHeader icon={AlertTriangle} title={t('card.gotchas')} count={parsed.discoveries.gotchas_discovered.length} />
                <ul className="space-y-0.5">
            ...
            {parsed.discoveries?.changed_files && parsed.discoveries.changed_files.length > 0 && (
              <div>
                <SectionHeader icon={FileCode} title={t('card.changedFiles')} count={parsed.discoveries.changed_files.length} />
            ...
            {parsed.discoveries?.file_insights && parsed.discoveries.file_insights.length > 0 && (
              <div>
                <SectionHeader icon={FileCode} title={t('card.fileInsights')} count={parsed.discoveries.file_insights.length} />
            ...
            {parsed.subtasks_completed && parsed.subtasks_completed.length > 0 && (
              <div>
                <SectionHeader icon={CheckCircle2} title={t('card.subtasksCompleted')} count={parsed.subtasks_completed.length} />
                <div className="flex flex-wrap gap-1.5 pl-4">
