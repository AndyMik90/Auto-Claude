...
export interface TaskAPI {
  ...
  onTaskStatusChange: (callback: (taskId: string, status: TaskStatus, projectId?: string, reviewReason?: import('../../shared/types').ReviewReason) => void) => () => void;
...
export const createTaskAPI = (): TaskAPI => ({
  ...
  onTaskStatusChange: (
    callback: (taskId: string, status: TaskStatus, projectId?: string, reviewReason?: import('../../shared/types').ReviewReason) => void
  ): (() => void) => {
    const handler = (
      _event: Electron.IpcRendererEvent,
      taskId: string,
