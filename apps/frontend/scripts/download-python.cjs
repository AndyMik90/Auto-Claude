...
function createSitePackagesLink(pythonDir, sitePackagesDir, platform) {
  ...
  // Internal: python-runtime/win-x64/python/Lib/site-packages
  // External: python-runtime/win-x64/site-packages
  // Relative: ../../site-packages (but we use absolute for reliability)

  // Build the .pth file content
  // Include the main site-packages and pywin32 subdirectories (Windows only)
  const pthLines = [
  ...
  // These are normally added by pywin32.pth, but that file is in the external site-packages
  // which Python doesn't process for .pth files
  if (platform === 'win32') {
    pthLines.push(path.join(sitePackagesDir, 'win32'));
    pthLines.push(path.join(sitePackagesDir, 'win32', 'lib'));
    pthLines.push(path.join(sitePackagesDir, 'Pythonwin'));
  }
