...
/**
 * Create a .pth file in Python's internal site-packages to link to our external site-packages.
 * This is necessary because:
 * 1. We bundle packages in a separate directory (resources/python-site-packages)
 * 2. Some packages like pywin32 have their own .pth files that add subdirectories to sys.path
 * 3. Python only processes .pth files in actual site-packages directories, not PYTHONPATH entries
 *
 * Without this, imports like `import pywintypes` fail because Python can't find win32/lib/pywintypes.py
 *
 * @param {string} pythonDir - Path to the extracted Python directory (e.g., python-runtime/win-x64/python)
 * @param {string} sitePackagesDir - Path to our external site-packages directory
 * @param {string} platform - Node.js platform (darwin, win32, linux)
 */
function createSitePackagesLink(pythonDir, sitePackagesDir, platform) {
  // Find Python's internal site-packages directory
  let internalSitePackages;

  if (platform === 'win32') {
    // Windows: python/Lib/site-packages
    internalSitePackages = path.join(pythonDir, 'Lib', 'site-packages');
  } else {
    // Unix: python/lib/python3.x/site-packages
    const libDir = path.join(pythonDir, 'lib');
    let pythonVersion = `python${PYTHON_VERSION.split('.').slice(0, 2).join('.')}`;

    // Try to find the actual Python version directory
    if (fs.existsSync(libDir)) {
      try {
        const entries = fs.readdirSync(libDir);
        const pythonVersionDir = entries.find(e => e.startsWith('python3.'));
        if (pythonVersionDir) {
          pythonVersion = pythonVersionDir;
        }
      } catch (err) {
        // Use default version on error
        console.warn(`[download-python] Could not read Python lib directory: ${err.message}. Using default version.`);
      }
    }

    internalSitePackages = path.join(libDir, pythonVersion, 'site-packages');
  }

  // Ensure the directory exists
  if (!fs.existsSync(internalSitePackages)) {
    fs.mkdirSync(internalSitePackages, { recursive: true });
  }

  // Calculate relative path from internal site-packages to external site-packages
  // Using relative paths ensures the .pth file works at runtime regardless of install location
  // Internal: python-runtime/win-x64/python/Lib/site-packages (Windows)
  // Internal: python-runtime/win-x64/python/lib/python3.x/site-packages (Unix)
  // External: python-runtime/win-x64/site-packages
  const relativeSitePackages = path.relative(internalSitePackages, sitePackagesDir);

  // Build the .pth file content using relative paths
  const pthLines = [
    '# Auto-generated by download-python.cjs',
    '# Links bundled site-packages to Python\'s module search path',
    relativeSitePackages,
  ];

  // Add pywin32-specific paths on Windows
  // Read from pywin32.pth if it exists, otherwise check for directories
  if (platform === 'win32') {
    const pywin32PthPath = path.join(sitePackagesDir, 'pywin32.pth');

    if (fs.existsSync(pywin32PthPath)) {
      // Read paths from pywin32.pth dynamically
      console.log('[download-python] Found pywin32.pth, reading paths...');
      try {
        const pywin32PthContent = fs.readFileSync(pywin32PthPath, 'utf-8');
        const pywin32Paths = pywin32PthContent
          .split(/\r?\n/)
          .map(line => line.trim())
          .filter(line => line && !line.startsWith('#') && !line.startsWith('import '));

        for (const p of pywin32Paths) {
          // Convert to relative path from internal site-packages
          const fullPath = path.join(sitePackagesDir, p);
          if (fs.existsSync(fullPath)) {
            pthLines.push(path.relative(internalSitePackages, fullPath));
          }
        }
      } catch (err) {
        console.warn(`[download-python] Could not read pywin32.pth: ${err.message}. Falling back to directory check.`);
        addPywin32PathsIfExist(pthLines, sitePackagesDir, internalSitePackages);
      }
    } else {
      // Fallback: add pywin32 paths if directories exist
      addPywin32PathsIfExist(pthLines, sitePackagesDir, internalSitePackages);
    }
  }

  const pthContent = pthLines.join('\n') + '\n';
  const pthPath = path.join(internalSitePackages, 'bundled-packages.pth');

  try {
    fs.writeFileSync(pthPath, pthContent);
    console.log(`[download-python] Created site-packages link: ${pthPath}`);
  } catch (err) {
    throw new Error(`[download-python] Failed to create .pth file at ${pthPath}: ${err.message}`);
  }

  // Log the paths being added
  for (const line of pthLines) {
    if (!line.startsWith('#')) {
      console.log(`[download-python]   -> ${line}`);
    }
  }
}

/**
 * Helper to add pywin32 paths if they exist on disk.
 * Used as fallback when pywin32.pth is not available.
 */
function addPywin32PathsIfExist(pthLines, sitePackagesDir, internalSitePackages) {
  const pywin32Dirs = ['win32', 'win32/lib', 'Pythonwin'];
  for (const dir of pywin32Dirs) {
    const fullPath = path.join(sitePackagesDir, dir);
    if (fs.existsSync(fullPath)) {
      pthLines.push(path.relative(internalSitePackages, fullPath));
    }
  }
...
async function downloadPython(targetPlatform, targetArch, options = {}) {
  ...
  if (!skipPackages) {
    if (!fs.existsSync(requirementsPath)) {
      ...
      // Create .pth file to link external site-packages to Python's internal site-packages
      // This ensures Python can find packages and process their .pth files (e.g., pywin32)
      const pythonDir = path.join(platformDir, 'python');
      createSitePackagesLink(pythonDir, sitePackagesDir, info.nodePlatform);
    }
  }
