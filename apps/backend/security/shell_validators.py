...
def validate_shell_c_command(command_string: str) -> ValidationResult:
    ...
    # Extract all commands from the inner command string
    inner_commands = extract_commands(inner_command)
    ...
    # Validate each command against the security profile
    for cmd in inner_commands:
        is_allowed, reason = is_command_allowed(cmd, profile)
        if not is_allowed:
            return (
                False,
                f"Command '{cmd}' inside shell -c is not allowed: {reason}",
            )
