...
def _find_glab_executable() -> str | None:
    ...
    if not is_windows():  # Unix-like systems (macOS, Linux)
        homebrew_paths = [
            ...
            "/usr/bin/glab",  # System install
            "/snap/bin/glab",  # Snap store
        ]
    ...
    if is_windows():
        windows_paths = [
            ...
            os.path.expandvars(r"%PROGRAMFILES%\GitLab\glab\glab.exe"),
            os.path.expandvars(r"%PROGRAMFILES(X86)%\GitLab\glab\glab.exe"),
            os.path.expandvars(r"%LOCALAPPDATA%\Programs\GitLab\glab\glab.exe"),
            # npm global installation (glab.cmd)
            os.path.join(
                os.path.expanduser("~"), "AppData", "Roaming", "npm", "glab.cmd"
            ),
            # Scoop package manager
            os.path.join(
                os.path.expanduser("~"), "scoop", "apps", "glab", "current", "glab.exe"
            ),
            # Chocolatey package manager
            os.path.expandvars(r"%PROGRAMDATA%\chocolatey\lib\glab\tools\glab.exe"),
        ]
