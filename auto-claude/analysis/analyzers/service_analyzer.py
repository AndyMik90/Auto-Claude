...
class ServiceAnalyzer(BaseAnalyzer):
    ...
    def __init__(self, service_path: Path, service_name: str):
        # For Xcode projects, check if there's a subdirectory with the same name
        # This is a common Xcode project structure: ProjectName/ProjectName/code
        xcode_subdir = service_path / service_name
        if xcode_subdir.exists() and xcode_subdir.is_dir():
            # Use the subdirectory as the actual path for analysis
    ...
    def _find_entry_points(self) -> None:
        ...
        # For Swift, also check common app structure files
        if not self.analysis.get("entry_point") and self.analysis.get("language") == "Swift":
            # Check for SwiftUI App file pattern
    ...
    def _detect_dependencies(self) -> None:
        ...
        if self._exists("package.json"):
            ...
            deps = []
            # Extract package dependencies from .package(url: "...", ...)
            import re
            for match in re.finditer(r'\.package\([^)]*url:\s*"[^"]*\/([^/"]+)(?:\.git)?"', content):
                deps.append(match.group(1))
            if deps:
                self.analysis["dependencies"] = deps[:20]
