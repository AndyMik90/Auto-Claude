...
must_haves:
  truths:
    - "PRReviewFinding has confidence, source_agents, and cross_validated fields"
  ...
  artifacts:
    - path: "apps/backend/runners/github/models.py"
      provides: "PRReviewFinding with confidence, source_agents, cross_validated fields"
...
Output: Updated PRReviewFinding model with cross-validation fields, confidence scoring and routing logic in parallel_orchestrator_reviewer.py, updated orchestrator prompt with confidence tier documentation.
</objective>
...
<task type="auto">
  <name>Task 0: Add cross-validation fields to PRReviewFinding model</name>
  <files>apps/backend/runners/github/models.py</files>
  <action>
Add three new fields to the PRReviewFinding dataclass to support confidence routing and cross-validation:

1. `confidence: float = 0.5` - Confidence score (0.0-1.0), defaults to medium confidence
2. `source_agents: list[str] = field(default_factory=list)` - Which agents reported this finding
3. `cross_validated: bool = False` - Whether multiple agents agreed on this finding
...
Update the `from_dict()` classmethod to include:
```python
...
Place the new fields after the existing validation fields (around line 241, after `validation_explanation`).
  </action>
  <verify>
Read apps/backend/runners/github/models.py and verify:
- PRReviewFinding has `confidence: float = 0.5` field
- PRReviewFinding has `source_agents: list[str]` field with default_factory
- PRReviewFinding has `cross_validated: bool = False` field
- to_dict() includes all three new fields
- from_dict() handles all three new fields with defaults
  </verify>
  <done>PRReviewFinding model extended with confidence, source_agents, and cross_validated fields</done>
</task>
...
1. Run: `apps/backend/.venv/bin/python -c "from runners.github.models import PRReviewFinding; f = PRReviewFinding(id='test', severity='high', category='security', title='t', description='d', file='f.py', line=1); print(f'confidence={f.confidence}, source_agents={f.source_agents}, cross_validated={f.cross_validated}')"`
2. Run: `apps/backend/.venv/bin/python -c "from runners.github.services.parallel_orchestrator_reviewer import ParallelOrchestratorReviewer; print('Import OK')"`
3. Verify no syntax errors in modified files
4. Grep for "_apply_confidence_routing" to confirm the method exists and is called
</verification>

<success_criteria>
- PRReviewFinding model has confidence, source_agents, and cross_validated fields
